...
// Pass fail for UC-2: ViewMarketMap
test('Select Market to view grid Map - UC-2: ViewMarketMap', () =>{

    expect(user.viewMarketMap('Stann Creek')).toBe(true)
})
...
test('Get Route - UC-3: GetMarketRoute ', () =>{
    ...
    expect(user.getMarketRoute('17.1573248','-89.0830848','17.1569824,','-89.0730777')).toBe(true)
...
test('Tour Market - UC-4: TourMarket',() =>{
    ...
    expect(user.tourMarket('Corozal')).toBe(true)
})
...
test('Tour Stall - UC-5: TourStall',() =>{
    ...
    expect(user.tourStall('C105')).toBe(true)
})
...
test('Buy Item - UC-7: BuyItem ', () =>{
    ...
    expect(user.buyItem(1002, 10011, 2)).toBe(true)
})
...
test('ApproveReservedStall - UC-15: ApproveReservedStall',() =>{
    ...
    expect(manageUser.approveReserveStall('v32','P5',auth('rRamos','%34k','management'))).toBe(true)
})
